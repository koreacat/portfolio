...
const Index = () => {
  ...
  const getTransitionType = (index: number) => {
    return index === transitionIndex ? TransitionContentEnum["fade-in"] : TransitionContentEnum["fade-out"];
  }
    ...
    <div className="relative w-full h-full">
      ...
      <Transition onTransitionStart={handleTransitionStart} onTransitionEnd={handleTransitionEnd}>
        <Transition.Scene className={open ? "bg-my-bg-color" : "bg-transparent"} style={TrasitionStyle}>
          <Scene0 transitionType={getTransitionType(0)} />
        </Transition.Scene>
        <Transition.Scene className={scene1ClassName} style={TrasitionStyle}>
          <Scene1 transitionType={getTransitionType(1)} />
        </Transition.Scene>
        <Transition.Scene className={scene2ClassName} style={TrasitionStyle}>
          <Scene2 transitionType={getTransitionType(2)} />
        </Transition.Scene>
        <Transition.Scene className={scene3ClassName} style={TrasitionStyle}>
          <Scene3 transitionType={getTransitionType(3)} />
        </Transition.Scene>
      </Transition>

      <TransitionContent type="fade-in" fadeInDelay="3s" className="absolute right-0 left-0 flex items-center justify-center flex-row gap-16 bottom-8 md:right-8 md:left-auto md:top-0 md:bottom-0 md:flex-col md:w-4">
        <Nav transitionIndex={transitionIndex} />
      </TransitionContent>

      <TransitionContent type={transitionIndex === 3 ? 'fade-out' : 'fade-in'} fadeInDelay="3s" className="hidden md:flex absolute w-4 flex items-center justify-end flex-col right-8 bottom-8">
        <Mouse />
      </TransitionContent>
    </div>
  )
